...
int write_data_corr_masked(
    ...
    float pow0_out =0, pow1_out=0, corr_re_out=0, corr_im_out=0;
    float power_diff = 0, phase_diff = 0;
    ...
    if(window<0){
        ...
        //here just sum everything 
        for(int i=0; i<window; ++i){
            pow0_out += power0[main_peak-window/2+i];
            pow1_out += power1[main_peak-window/2+i];
            corr_re_out += corr_re[main_peak-window/2+i];
            corr_im_out += corr_im[main_peak-window/2+i];
        }
        phase_diff = std::atan2(corr_im_out, corr_re_out)*180/M_PI;
        power_diff = std::sqrt(pow0_out/pow1_out);

        file.write(reinterpret_cast<const char*>(&timestamp), sizeof(timestamp));
        file.write(reinterpret_cast<const char*>(&power_diff),  sizeof(float));
        file.write(reinterpret_cast<const char*>(&phase_diff),  sizeof(float));
        return 0;
    }
}
